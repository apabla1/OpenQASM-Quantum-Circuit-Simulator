...
3. The token list is then passed to the parser (`parser.py`), which converts the token list to a program representation with (i) the number of qubits (where unused qubits are optimized out), (ii) the number of classical bits, and (iii) the ordered-operation list by using the following grammar:
...
5. The optimized final state is then converted to the original number of  qubits and is then reported. The simulator converts the state from a list of weighted to to a trimmed, vector (numpy array) form.
